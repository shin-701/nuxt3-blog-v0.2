...
<template>
  <nav
    class="md:hidden fixed bottom-0 left-0 right-0 h-20 bg-m3-surface-container border-t border-m3-outline-variant flex items-center justify-around px-2 z-50"
  >
    <!-- Home -->
    <NuxtLink
      to="/"
      class="flex flex-col items-center justify-center gap-1 px-4 py-2 rounded-fab m3-state-layer transition-all duration-200 min-w-[64px]"
    ...
    >
      ...
      <span
        ...
        :class="
          route?.path === '/'
            ? 'text-m3-on-secondary-container font-medium'
            : 'text-m3-on-surface-variant'
        "
      >
        Home
    ...
    <Transition name="slide-up">
      <div
        v-if="isSocialOpen"
        class="absolute bottom-24 right-4 bg-m3-surface-container-high rounded-card shadow-m3-3 p-4 flex flex-col gap-3"
      >
        <a
          href="https://x.com/shinTasomaRu"
          target="_blank"
          rel="noopener noreferrer"
          class="flex items-center gap-3 px-4 py-2 rounded-fab m3-state-layer hover:bg-m3-surface-container-highest transition-all duration-200"
          @click="isSocialOpen = false"
        >
          <Icon name="mdi:twitter" class="w-5 h-5 text-m3-on-surface" />
        ...
        <a
          ...
          @click="isSocialOpen = false"
        >
          <Icon name="mdi:github" class="w-5 h-5 text-m3-on-surface" />
