...
<template>
  <div v-if="post" class="min-h-screen bg-m3-surface">
    <!-- M3風のシンプルなヘッダー -->
    <header
      class="border-b border-m3-outline-variant bg-m3-surface py-4 sm:py-6"
    >
      <div class="container mx-auto px-4 sm:px-6 max-w-7xl">
        ...
        <h1
          class="text-headline-large sm:text-display-small font-medium text-m3-on-surface mt-2"
        >
          {{ post.title }}
        </h1>
        <p
          v-if="post.description"
          class="text-body-medium text-m3-on-surface-variant mt-2"
        >
          {{ post.description }}
        </p>
        ...
        >
          <span
            ...
            :key="tag"
            class="inline-flex items-center px-3 py-1 rounded-full bg-m3-secondary-container text-m3-on-secondary-container text-label-small font-medium"
          >
            {{ tag }}
          </span>
        </div>
      ...
      <div class="container mx-auto px-6 sm:px-8 lg:px-12 xl:px-16 py-8">
        <div class="flex gap-8 xl:gap-12 max-w-6xl mx-auto">
          ...
          <article class="flex-1 min-w-0">
            <ContentRenderer :value="post" class="prose prose-m3 max-w-none" />
          </article>
